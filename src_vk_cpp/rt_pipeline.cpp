...
RtPipeline::RtPipeline(VkContext& ctx, VkDescriptorSetLayout rt_output_layout) {
    ...
    // Stage indices: 0=rgen, 1=miss, 2=opaque_chit, 3=glass_chit, 4=glass_ahit
    ...
    stages[3].module = glass_chit_mod;
    stages[3].pName  = "main";

    stages[4].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    stages[4].stage  = VK_SHADER_STAGE_ANY_HIT_BIT_KHR;
    stages[4].module = glass_ahit_mod;
    stages[4].pName  = "main";

    // ------------------------------------------------------------------ Shader groups
    // Group 0: raygen (general group, raygen stage)
    // Group 1: miss   (general group, miss stage)
    // Group 2: opaque hit (triangles, closest-hit stage 2)        ← sbt_record_offset 0
    // Group 3: glass hit  (triangles, closest-hit 3, any-hit 4)   ← sbt_record_offset 1
    ...
    groups[2].closestHitShader   = 2;  // opaque chit
    ...
    groups[3].closestHitShader   = 3;  // glass chit
    groups[3].anyHitShader       = 4;  // glass ahit
    ...
    pipe_ci.stageCount                   = 5;
    pipe_ci.pStages                      = stages;
    pipe_ci.groupCount                   = 4;
    ...
    // Regions: raygen (group 0), miss (group 1), hit (groups 2+3 — opaque then glass).
    ...
    // Fetch all four handles (rgen, miss, opaque_hit, glass_hit).
    std::vector<uint8_t> handles(4 * handle_size);
    if (ctx.pfn_vkGetRayTracingShaderGroupHandlesKHR(device, pipeline,
                                                     0, 4, handles.size(), handles.data()) != VK_SUCCESS)
    ...
    std::memcpy(data + hit_offset,                   handles.data() + 2 * handle_size, handle_size); // opaque
    std::memcpy(data + hit_offset + handle_stride,   handles.data() + 3 * handle_size, handle_size); // glass
    ...
    hit_region.size          = 2 * handle_stride;  // opaque + glass
